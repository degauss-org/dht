...
geomarker <- basename(here::here()) # find: paste_geomarker_name_here
container_version <- 0.0.1
r_version <- dht::detect_r_version() # find: paste_r_version 
renv_version <- packageVersion("renv") # find: paste_renv_version
```

```{text dockerfile, eval = FALSE, echo = FALSE}
FROM rocker/r-ver:paste_r_version
...
ENV RENV_VERSION paste_renv_version
...
COPY paste_geomarker_name_here.rds .
COPY entrypoint.R .
...
dht::greeting(geomarker_name = 'paste_geomarker_name_here', version = '0.0.1', description = 'short description goes here')
...
get_paste_geomarker_name_here <- function(query_point) {{
...
d <- d %>%
   mutate(paste_geomarker_name_here = mappp::mappp(d$d$geometry, get_paste_geomarker_name_here,
                                   parallel = FALSE,
                                   quiet = FALSE))
...
write_geomarker_file(d = d$d,
                     ...
                     geomarker_name = 'paste_geomarker_name_here',
                     version = '0.0.1')"
```
...
jobs:
  deploy-images:
    ...
    env:
      ...
      repository: paste_geomarker_name_here
...
# paste_geomarker_name_here <a href='https://degauss-org.github.io/DeGAUSS/'><img src='https://github.com/degauss-org/degauss_template/blob/master/DeGAUSS_hex.png' align='right' height='138.5' /></a>

> short description of paste_geomarker_name_here

[![Docker Build Status](https://img.shields.io/docker/automated/degauss/paste_geomarker_name_here)](https://hub.docker.com/repository/docker/degauss/paste_geomarker_name_here/tags)
[![GitHub Latest Tag](https://img.shields.io/github/v/tag/degauss-org/paste_geomarker_name_here)](https://github.com/degauss-org/paste_geomarker_name_here/releases)
...
```sh
docker run --rm -v $PWD:/tmp degauss/paste_geomarker_name_here:0.0.1 my_address_file_geocoded.csv
```

will produce `my_address_file_geocoded_paste_geomarker_name_here_v0.0.1.csv` with an added column named paste_geomarker_name_here.
...
- list where geomarker data is stored in S3 using a hyperlink like: [`s3://path/to/paste_geomarker_name_here.rds`](https://geomarker.s3.us-east-2.amazonaws.com/path/to/paste_geomarker_name_here.rds)
