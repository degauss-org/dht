...
greeting <- function(geomarker_name, version, description) {
  cli::cli_h1('Wecome to DeGAUSS!', wrap = TRUE)
  cli::cli_alert_info('You are using the {geomarker_name} container, version {version}.', wrap = TRUE)
  cli::cli_text('This container {description}.', wrap = TRUE)
  cli::cli_text('For more information about the {geomarker_name} container,
                visit {.url https://degauss.org/{geomarker_name}/}', wrap = TRUE)
  cli::cli_text('For DeGAUSS troubleshooting, visit {.url https://degauss.org/}', wrap = TRUE)
  cli::cli_text('To help us improve DeGAUSS, please take our user survey at {.url https://redcap.link/jf4lil0n}', wrap = TRUE)
}
